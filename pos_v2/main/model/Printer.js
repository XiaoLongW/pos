function Printer(carts) {
  ...
  this.print = function () {
    ...
    for (var x = 0; x < this.carts.length; x++) {
      ...
      str += '名称：' + this.carts[x].name + '，数量：' + this.carts[x].relCount + this.carts[x].unit + '，单价：' + this.carts[x].price.toFixed(2) + '(元)，小计：' + (this.carts[x].payCount * this.carts[x].price).toFixed(2) + '(元)\n';
    }
    str += '----------------------\n挥泪赠送商品：\n';
    for (var y = 0; y < this.carts.length; y++) {
      if (this.carts[y].payCount != this.carts[y].relCount) {
        this.saveMoney += this.carts[y].price * ( this.carts[y].relCount - this.carts[y].payCount);
        str += '名称：' + this.carts[y].name + '，数量：' + ( this.carts[y].relCount - this.carts[y].payCount) + this.carts[y].unit + '\n';
      }
    }
    str += '----------------------\n';
